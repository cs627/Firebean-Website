 ...
 *     → Select row(s) → click button → marks them "Pending (images)"
 *     → Then click "Sync Changed Only" to push
...
function doSync(changedOnly) {
  ...
  var idMap = {};  // projectId → [rowNumbers]
  for (var d = 1; d < data.length; d++) {
    var dProjectName = String(data[d][CONFIG.COL.PROJECT - 1] || '').trim();
    if (!dProjectName) continue;
    var dProjectId = String(data[d][CONFIG.COL.PROJECT_ID - 1] || '').trim();
    if (!dProjectId) continue;
    if (!idMap[dProjectId]) idMap[dProjectId] = [];
    idMap[dProjectId].push({row: d + 1, name: dProjectName});
  }
  var dupes = [];
  for (var did in idMap) {
    if (idMap[did].length > 1) {
      var names = idMap[did].map(function(e) { return 'Row ' + e.row + ' (' + e.name + ')'; }).join(', ');
      dupes.push(did + ': ' + names);
    }
  }
  if (dupes.length > 0) {
    var dupeMsg = '⚠️ DUPLICATE Project IDs detected!\n\n' +
      dupes.join('\n') +
      '\n\nDuplicate IDs will cause image conflicts.\nPlease fix before syncing.';
    Logger.log(dupeMsg);
    try {
      var dupeResult = SpreadsheetApp.getUi().alert('Duplicate Project IDs', dupeMsg, SpreadsheetApp.getUi().ButtonSet.OK_CANCEL);
      if (dupeResult !== SpreadsheetApp.getUi().Button.OK) return;
    } catch(e) {}
  }
