/**
 * newsletter-feed.js — Populates "The Dispatch" newsletter cards
 * with latest EDM market insight content from data/edm-feed.json.
 * Links open in new tab since EDM pages have different branding.
 * Runs after CMS inline code to overwrite with EDM content.
 */
(function() {
  'use strict';

  var BASE = (document.querySelector('base[href]') ? document.querySelector('base[href]').getAttribute('href') : '/');
  var FEED_URL = BASE + 'data/edm-feed.json';

  function truncateToWords(text, maxWords) {
    if (!text) return '';
    var words = text.split(/\s+/);
    if (words.length <= maxWords) return text;
    return words.slice(0, maxWords).join(' ') + '…';
  }

  function populateCards(feed) {
    for (var i = 0; i < Math.min(feed.length, 3); i++) {
      var n = i + 1;
      var entry = feed[i];
      
      var img = document.getElementById('newsletter-img-' + n);
      var badge = document.getElementById('newsletter-badge-' + n);
      var dateEl = document.getElementById('newsletter-date-' + n);
      var title = document.getElementById('newsletter-title-' + n);
      var desc = document.getElementById('newsletter-desc-' + n);
      var link = document.getElementById('newsletter-link-' + n);
      
      if (img && entry.photo) {
        img.src = entry.photo;
        img.alt = entry.title || 'Report';
      }
      if (badge && entry.issue) {
        badge.textContent = entry.issue;
      }
      if (dateEl && entry.date) {
        dateEl.textContent = entry.date;
      }
      if (title && entry.title) {
        title.textContent = entry.title;
      }
      if (desc && entry.desc) {
        desc.textContent = truncateToWords(entry.desc, 50);
      }
      if (link && entry.url) {
        link.href = entry.url;
        link.setAttribute('target', '_blank');
        link.setAttribute('rel', 'noopener');
      }
    }
  }

  function loadFeed() {
    try {
      var xhr = new XMLHttpRequest();
      xhr.open('GET', FEED_URL, true);
      xhr.onload = function() {
        if (xhr.status === 200) {
          try {
            var feed = JSON.parse(xhr.responseText);
            if (feed && feed.length) { populateCards(feed); return; }
          } catch(e) {}
        }
        useFallback();
      };
      xhr.onerror = useFallback;
      xhr.send();
    } catch(e) { useFallback(); }
  }

  function useFallback() {
    var fallback = [
      { issue:"EDM_008", date:"September 1, 2026", title:"Heritage Meets Innovation: Lessons from ICH Month 2026", desc:"75% retention vs. 5% from lectures. Immersive storytelling isn't a trend — it's the new baseline for public engagement. Learning That Actually Sticks — Hong Kong's cultural institutions are discovering that audiences remember immersive experiences at dramatically higher rates.", photo:"https://raw.githubusercontent.com/cs627/Firebean-Website/main/assets/edm/EDM_008/s2_immersive_storytelling.jpg", url:"https://firebean.net/edm/edm_EDM_008.html" },
      { issue:"EDM_007", date:"August 25, 2026", title:"Inclusive Hong Kong: Universal Design & Experiential Accessibility", desc:"Turn event foot traffic into actionable public sentiment data. Traditional 'head-counting' at events is being replaced by sophisticated sentiment analysis. Government departments can now measure not just how many people attended, but how they felt about the policy message.", photo:"https://raw.githubusercontent.com/cs627/Firebean-Website/main/assets/edm/EDM_007/insight.jpg", url:"https://firebean.net/edm/edm_EDM_007.html" },
      { issue:"EDM_006", date:"August 18, 2026", title:"Fortifying Public Trust: Data Privacy & Security in Gov Engagements", desc:"Combine the impact of physical events with the reach of digital platforms. The 'Phygital' (Physical + Digital) model is the new standard for civic engagement. In 2026, the most successful government campaigns use physical touchpoints to drive traffic to digital platforms.", photo:"https://raw.githubusercontent.com/cs627/Firebean-Website/main/assets/edm/EDM_006/insight.jpg", url:"https://firebean.net/edm/edm_EDM_006.html" }
    ];
    populateCards(fallback);
  }

  // Wait for CMS inline code to run first, then overwrite
  function init() {
    setTimeout(loadFeed, 500);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(loadFeed, 800);
    });
  } else {
    init();
  }
})();